...
const realtimeContent = ref(content.scenes[ScenesChat.scenesIndex].student[0]);
const isOpen = ref(true)
const lastTeacherCnt = ref(-1)
const teacherCnt = ref(0)

const teacherChange = computed(()=>ScenesChat.teacherBubble);
watch(teacherChange,async (newVal,oldVal)=>{
	console.log("老师的变化了",ScenesChat.teacherBubble)
	console.log("学生该说：",content.scenes[ScenesChat.scenesIndex].student[ScenesChat.teacherBubble])
	realtimeContent.value = content.scenes[ScenesChat.scenesIndex].student[ScenesChat.teacherBubble]
	isOpen.value = true
})
...
<template>
    <!-- 当未点击关闭 和 老师讲完后 出现tips -->
    <div class="tips-popover" v-if="isOpen && teacherCnt != lastTeacherCnt">
            <img class="tips-icon" src="@/assets/scenesChat/tips.png">
            <div class="tips-prefix">YOU CAN SAY AS FOLLOWS:<br>
                <span class="tips-content">{{ realtimeContent }} </span>
            </div>
        <img src="@/assets/scenesChat/close.png" alt="close" class="close-icon"  @click="closeTips()">
    </div>
</template>
<style scoped>
.tips-popover {
