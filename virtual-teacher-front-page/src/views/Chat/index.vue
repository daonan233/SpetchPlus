<template>
  ...
  <div class="mainArea">
    ...
    <div class="feedback-box" v-if="StudyInfoStore.clickedIndex != -1 && !StudyInfoStore.sentencesDetail[StudyInfoStore.clickedIndex].isTexing">
...
<script setup>
import VTeacher from "@/views/Chat/Components/VTeacher.vue";
import ChatBox from "@/views/Chat/Components/ChatBox.vue";
import LearningFeedback from "@/views/Chat/Components/LearningFeedback.vue";
...
const clickedIndex = computed(() => StudyInfoStore.clickedIndex + 1);
...
function showPopover(text) {
  ...
  // 将输入框中的内容添加进聊天记录中
  StudyInfoStore.userChatContent.push(recognization);
...
async function onRecognizedResult(result) {
  ...
  // 输出单词级别的详细信息，包括单词、准确度分数、错误类型等
  // console.log("Word-level details:");
  // _.forEach(pronunciation_result.detailResult.Words, (word, idx) => {
  //   console.log("-----------phoneme-level details:");
  //   _.forEach(word.Phonemes, (phoneme, idx) => {
  //     console.log("    ", idx + 1, ": phoneme: ", phoneme.Phoneme, "\taccuracy score: ", phoneme.PronunciationAssessment.AccuracyScore + ";");
  //   });
  //   console.log("-----------syllable-level details:");
  //   _.forEach(word.Syllables, (syllable, idx) => {
  //     console.log("    ", idx + 1, ": syllable: ", syllable.Syllable, "\taccuracy score: ", syllable.PronunciationAssessment.AccuracyScore + ";");
  //   });
...
async function startMicRecording() {
  ...
  mediaRecorder.onstop = () => {
    ...
    // 若不确定两者的结束顺序，则可以再次判断是否为空，为空则赋值
    // ---------（考虑删除掉）
    if (StudyInfoStore.sentencesDetail[StudyInfoStore.sentencesDetail.length - 1].sentenceAudioUrl === '') {
      StudyInfoStore.sentencesDetail[StudyInfoStore.sentencesDetail.length - 1].sentenceAudioUrl = audioUrl;
    }
  };
...
let teacherInput = ref('');

// 监听用户输入的文本，若有新输入，则进入LLM
watch(() => StudyInfoStore.userChatContent, () => {
  console.log("new content: " + StudyInfoStore.userChatContent[StudyInfoStore.userChatContent.length - 1]);

  // 55，你使用时，打开这里（现在不打开时因为打开后报错，气泡会不显示）
  // teacherInput.value = StudyInfoStore.userChatContent[StudyInfoStore.userChatContent.length - 1];
