...
// 初始化反馈界面所需要的分数等内容
const clickedIndex = StudyInfoStore.clickedIndex;
console.log("重新渲染了嘛??clickedIndex:",clickedIndex);

// const clickedIndex = computed(() => StudyInfoStore.clickedIndex);
let totalScore = StudyInfoStore.sentencesDetail[clickedIndex]?.pronunciationScore;
let accuracyScore = StudyInfoStore.sentencesDetail[clickedIndex]?.accuracyScore;
let fluencyScore = StudyInfoStore.sentencesDetail[clickedIndex]?.fluencyScore;
let completenessScore = StudyInfoStore.sentencesDetail[clickedIndex]?.completenessScore;
let words = StudyInfoStore.sentencesDetail[clickedIndex]?.wordsArray.map((item) => item.word);
let wordsDetail = StudyInfoStore.sentencesDetail[clickedIndex]?.wordsArray;
...
const closeFeedback = () => {
    ...
    StudyInfoStore.clickedIndex = -1;
}

// 播放用户单词音频
function playWordAudio() {
    const audioUrl = StudyInfoStore.sentencesDetail[clickedIndex].sentenceAudioUrl;
    if (audioUrl) {
...
.show-pronunciation-title::after {
    content: '';
